...
local external_monitor = "DP-1"
hl.monitor({ output = external_monitor, mode = "highrr", position = "0x0", scale = 1 })
hl.monitor({ output = laptop_monitor, mode = "highrr", position = "2560x0", scale = 1 })
...
hl.bind(
	"SUPER + S",
	hl.dsp.exec_cmd(
		'grim -g "$(slurp)" ~/Pictures/Screenshots/$(date +"Screenshot_%Y-%m-%d_%H-%M-%S.png") && ~/.config/hypr/scripts/screenshot_notify'
	)
)
...
hl.bind(
	"XF86MonBrightnessUp",
	hl.dsp.exec_cmd("brightnessctl -d 'intel_backlight' set 5%+ && ~/.config/hypr/scripts/brightness_notify")
)
hl.bind(
